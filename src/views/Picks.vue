  ...
  <div class="container">
    ...
    <table class="table" align="center" style="width: 750px">
      ...
      <tbody v-for="pick in picks">
        <tr>
          <th scope="row">{{ pick.users_pick }}</th>
          <td>{{ pick.confidence_point }}</td>
        </tr>
      </tbody>
    </table>
...
export default {
  ...
  created: function() {
    axios.get('https://nba-confidence-pick-em-league.herokuapp.com/api/user_games').then(
      function(response) {
        console.log(response.data);
        this.picks = response.data['user_games'];
      }.bind(this)
    );
  },
  methods: {},
